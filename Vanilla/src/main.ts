...
class PermanentBeautyWebsite {
    ...
    private validateForm(): boolean {
        ...
        else formData.name = nameInput.value;

        // Email validation
        const emailInput = this.contactForm.querySelector('#email') as HTMLInputElement;
        if (!this.validateField(emailInput)) isValid = false;
        else formData.email = emailInput.value;

        // Phone validation
        const phoneInput = this.contactForm.querySelector('#phone') as HTMLInputElement;
        if (!this.validateField(phoneInput)) isValid = false;
        else formData.phone = phoneInput.value;

        // Service validation
        const serviceInput = this.contactForm.querySelector('#service') as HTMLSelectElement;
        if (!this.validateField(serviceInput)) isValid = false;
        else formData.service = serviceInput.value;
    ...
    private validateField(field: HTMLInputElement | HTMLSelectElement): boolean {
        ...
        const errorElement = document.querySelector(`#${field.id}Error`) as HTMLElement;
    ...
    private showError(field: HTMLInputElement | HTMLSelectElement, errorElement: HTMLElement | null, message: string): void {
        if (errorElement) {
    ...
    private submitForm(): void {
        ...
        const successMessage = document.querySelector('#formSuccess') as HTMLElement;
        if (successMessage) {
    ...
    // Gallery Filtering
    private setupGalleryFilter(): void {
    ...
    private filterGalleryItems(filter: string | null): void {
